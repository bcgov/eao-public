  ...
  describe('given a valid array', () => {
    ...
    describe('of non-unique items', () => {
      ...
      it('that have the same region value as the value passed in', () => {
        expect(JSON.stringify(pipe.transform(value, string)[0].region))
        .toBe(JSON.stringify(expectedResponse[0].region));
        expect(JSON.stringify(pipe.transform(value, string)[1].region))
        .toBe(JSON.stringify(expectedResponse[1].region));
      });
    });
